...
function startGame () {
        ...
        $("#etape5").click(function () {
            ...
            for (i = 0; i < defenseur.length; i++) {
                ...
                for (let heroSoins of listJoueur[index].Maison.listHero.filter(heroSoin => heroSoin.Type == Fonction.Soin)) {
                    CombatTS.listCombat.filter(fight => fight.IdHouseDefense === parseInt(resDefenseur[1]) && fight.IdHouseAttack === parseInt(resDefenseur[2]))[0].ListHeroSoins.push(heroSoins.IdCharacter);
                }
            }
            index++;
            if (index < listJoueur.length) {
    ...
    function reset() {
        ...
        $("#listJoueur").empty();
        for (let joueur of listJoueur) {
            if (HouseTS.listMaison.filter(house => house.IdHouse == joueur.Maison.IdHouse).length == 0) {
